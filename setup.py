...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_polar", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_polar": ["*.pyi", "py.typed"],
        "tree_sitter_polar.queries": ["*.scm"],
    },
    ext_package="tree_sitter_polar",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_polar/binding.c",
                "src/parser.c",
            ],
            define_macros=[
